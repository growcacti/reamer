...
class Ctrl_Tabs:
    ...
    def add_widgets_to_tab(self, index, canvas):
        if index == 1:
            ...
            tk.Label(canvas, text="Replace:").grid(row=2,column=2)
            self.match_entry = tk.Entry(canvas)
            self.match_entry.grid(row=3, column=3)

            # Entry for replace pattern
            tk.Label(canvas, text="With:").grid(row=4,column=3)
            self.replace_entry = tk.Entry(canvas)
            self.replace_entry.grid(row=5, column=3)

            # Apply button
            self.apply_button = tk.Button(canvas, text="Apply", command=self.apply_replacement)
            self.apply_button.grid(row=7,column=3)


        elif index == 4:
            tk.Label(canvas, text="Enumerate:").grid(row=2,column=2)
           

            # Entry for replace pattern
            tk.Label(canvas, text="Count:").grid(row=4,column=3)
            

            # Apply button
            self.a_button = tk.Button(canvas, text="Apply",bd=7,bg="orange", command = lambda: self.numbering(self.fn.lb1.curselection()))
            self.a_button.grid(row=7,column=3)
