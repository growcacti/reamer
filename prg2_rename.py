...
class MainApplication(tk.Tk):
    def __init__(self):
        ...
        dirnav = DirectoryTreeNavigator(self.main_frame, self.initial_path, self.fn.refresh_view)
       ...
       # Menubar setup
        ...
        self.file_menu.add_command(
            label="Refresh View", underline=1, command=lambda: self.fn.refresh_view(initial_path)
        )
       
        self.prefix_suffix_btn = ttk.Button(self.main_frame, text="Add Prefix/Suffix", command=self.open_prefix_suffix_dialog)
        self.prefix_suffix_btn.grid(row=2, column=0, padx=5, pady=5)
